  ...
  methods: {
    ...
    getMeasurement: function getMeasurement(id) {
      ...
      axios.get("/product/measurements/" + id).then(function (response) {
        ...
        for (var i = 0; i < _this5.measurements.length; i++) {
          if (_this5.measurements[i].meas_pk == _this5.detail.meas_fk) {
            _this5.selectmeas = _this5.measurements[i];
          }
        }
      })["catch"](function (e) {
        console.log(e);
    ...
    agregar: function agregar(item) {
      ...
      axios.post('/provider/purchase/details', this.detail).then(function (response) {
        console.log(_this6.detail);

        if (response.data.status.code == 200) {
          if (_this6.directa == 2) {
            _this6.prpo_pk = response.data.data;
          }

          _this6.normal('Notificación', '¡Producto Agregado correctamente!', "success");
      ...
      if (this.directa == 1) {
        axios.post('/provider/purchases?prpo_pk=' + this.prpo_pk + '').then(function (response) {
          ...
          if (response.data.data != null) {
            ...
            _this8.prpu_pk = response.data.data.ProviderPurchase.prpu_pk;
            _this8.prov_pk = response.data.data.ProviderPurchase.prov_fk; //response.data.data.ProviderPurchaseDetail//
          ...
          //this.errors.push(e)
          console.log(e); //this.normal('Notificación', "Error al cargar los datos11", "error");
    ...
    getTotal: function getTotal() {
      ...
      this.total = this.subtotal; //console.log('this.total = ' +  this.total)
    },
