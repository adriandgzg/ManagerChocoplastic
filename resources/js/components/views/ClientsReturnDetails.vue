          ...
          <v-card>
            <v-form v-model="valid">
              <v-card-title class="subheading font-weight-bold"
                ><H2>{{ saleHeader.prpu_identifier }}</H2></v-card-title
              >
              <v-divider></v-divider>
              <v-row>
                <v-col cols="4">
              ...
              <template v-slot:default>
                ...
                <tbody>
                  <tr v-for="item in desserts" :key="item.prod_name">
                    ...
                    <td>
                      ${{ formatMoney(item.prrd_quantity * item.prrd_price) }}
                    </td>
                    <td>
...
export default {
  ...
  methods: {
    ...
    finalizar() {
      ...
      if (r == true) {
        axios
          .post("/provider/returns/update", this.editadoSale)
          .then((response) => {
    ...
    onQuantityChange(item) {
      ...
      axios
        .post("/provider/return/details/update", this.editado)
        .then((response) => {
          console.log(response);
          //this.textMsg = "¡Actualizado correctamente!";
    ...
    createsale() {
      ...
      axios
        .post("/provider/returns?prpu_pk=" + this.prpu_pk + "")
        .then((response) => {
    ...
    getTotal() {
      for (var i = 0; i < this.desserts.length; i++) {
        this.subtotal =
          this.subtotal +
          this.desserts[i].prrd_price * this.desserts[i].prrd_quantity;
      }
    ...
    delete: function () {
      axios
        .post("/provider/return/details/destroy", this.editado)
        .then((response) => {
