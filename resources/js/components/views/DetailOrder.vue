              ...
              <template v-slot:default>
                ...
                <tbody>
                  <tr v-for="item in desserts" :key="item.prod_name">
                    ...
                    <td>
                      <v-text-field
                        v-model="item.clsd_discountrate"
                        label
                        @change="onQuantityChange(item)"
                        required
                      ></v-text-field>
        ...
        <v-card>
          ...
          <v-card-text>
            <v-form v-model="valid">
              ...
              <v-row>
                <v-col cols="6">
                  <v-text-field
                    required
                    label="Monto: "
                    v-model="editadoPago.cpam_amount"
              ...
              <v-btn color="primary" :disabled="!valid" @click="agregarPago">Agregar</v-btn>
            </v-form>
          </v-card-text>
        </v-card>
      </v-dialog>
...
export default {
  ...
  methods: {
    ...
    finalizar() {
      ...
      else this.editadoSale.stor_fk = this.users.store_id;
    ...
    createsale() {
      this.loading = true;
      axios
        .post("/clientsales?clor_pk=" + this.clor_pk + "")
        .then((response) => {
    ...
    agregarPago() {
      ...
      axios
        .post("/client/payment/amounts", this.editadoPago)
        .then((response) => {
          setTimeout(() => (this.loading = false), 500);
          console.log(response.data);
          if (response.data.data != null) {
    ...
    getTotal() {
      ...
      for (var i = 0; i < this.desserts.length; i++) {
        this.subtotal =
          this.subtotal +
          this.desserts[i].clsd_price * this.desserts[i].clsd_quantity;
      }
    ...
    getEfectivo() {
      ...
      for (var i = 0; i < this.pagos.length; i++) {
        ...
        if (this.pagos[i].pash_name == "Efectivo") {
          this.efectivo =
            parseFloat(this.efectivo) + parseFloat(this.pagos[i].cpam_amount);
        }
        this.montototal =
          parseFloat(this.montototal) + parseFloat(this.pagos[i].cpam_amount);
      }
      this.getcambio();
    },
    getClients() {
    ...
    guardaBorrarPago() {
      ...
      axios
        .post("/client/payment/amounts/destroy", this.editadoPago)
        .then((response) => {
    ...
    delete: function () {
      axios
        .post("/client_sale_details/destroy", this.editado)
        .then((response) => {
          this.textMsg = "¡Eliminado correctamente!";
          this.normal("Notificación", this.textMsg, "success");
          this.createsale();
