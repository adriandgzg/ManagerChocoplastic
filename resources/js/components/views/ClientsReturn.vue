    ...
    <v-container>
      ...
      <v-dialog v-model="dialogQuestion" persistent max-width="290">
        <v-card>
          ...
          <v-card-actions>
            <v-spacer></v-spacer>
            <v-btn color="green darken-1" text @click="dialogQuestion = false"
              >Cancelar</v-btn
            >
            <v-btn color="green darken-1" text @click="guardaFinalizar"
              >Continuar</v-btn
            >
          </v-card-actions>
        </v-card>
      ...
      <v-row>
        <v-col>
          <v-card>
            <v-form v-model="valid">
              <v-card-title class="subheading font-weight-bold"
                ><H2>{{ saleHeader.clsa_identifier }}</H2></v-card-title
              >
              <v-divider></v-divider>
              <v-row>
                <v-col cols="4">
          ...
          <v-card justify="end">
            <v-btn :disabled="!valid" dark color="success" @click="finalizar"
              >FINALIZAR</v-btn
            >
          </v-card>
        </v-col>
              ...
              <template v-slot:default>
                ...
                <tbody>
                  <tr v-for="item in desserts" :key="item.prod_name">
                    ...
                    <td>
                      ${{ formatMoney(item.clrd_quantity * item.clrd_price) }}
                    </td>
                    <td>
...
export default {
  ...
  methods: {
    ...
    guardaFinalizar() {
      ...
      axios
        .post("/client/returns/update", this.editadoSale)
        .then((response) => {
    ...
    onQuantityChange(item) {
      ...
      if (this.editado.clrd_quantity > this.editado.clrd_quantity_sale) {
        this.textMsg =
          "¡El cantidad devuelta no puede ser mayor a la cantidad comprada!";
      ...
      axios
        .post("/client/return/details/update", this.editado)
        .then((response) => {
          console.log(response);
    ...
    createsale() {
      this.loading = true;
      axios
        .post("/client/returns?clsa_pk=" + this.clsa_pk + "")
        .then((response) => {
    ...
    getTotal() {
      for (var i = 0; i < this.desserts.length; i++) {
        this.subtotal =
          this.subtotal +
          this.desserts[i].clrd_price * this.desserts[i].clrd_quantity;
      }
    ...
    delete: function () {
      axios
        .post("/client/return/details/destroy", this.editado)
        .then((response) => {
